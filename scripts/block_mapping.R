...
# Louisiana census blocks
# Source: https://redist.legis.la.gov/default_ShapeFiles2020
blocks <- st_read("data/shapemaps/2025 Block Shapefile (01-16-2025)/_2025_01_Census_Blocks.shp")
...
senate <- st_read("data/shapemaps/tl_2025_22_sldl/tl_2025_22_sldl.shp")

# State House districts
house <- st_read("data/shapemaps/tl_2025_22_sldu/tl_2025_22_sldu.shp")
...
block_equivalency <- read_csv("data/voting_data/LA_2025_01_BLOCK_DATA.txt")
...
blocks <- st_transform(blocks, planar_crs) %>% 
  st_make_valid() %>% 
  st_buffer(0) %>%
  left_join(block_equivalency, by = "UNITNUM")
...
# Function to assign district based on largest area overlap
assign_district_by_area <- function(blocks_sf, districts_sf, district_col) {
  intersections <- st_intersection(blocks_sf, districts_sf %>% select(all_of(district_col)))
...
# Assign districts based on area weighting
congressional_assignment <- assign_district_by_area(blocks, congressional, "NAMELSAD") %>%
  rename(congressional = NAMELSAD)

senate_assignment <- assign_district_by_area(blocks, senate, "NAMELSAD") %>%
  rename(senate = NAMELSAD)

house_assignment <- assign_district_by_area(blocks, house, "NAMELSAD") %>%
  rename(house = NAMELSAD)

psc_assignment <- assign_district_by_area(blocks, public_service_commission, "NAME") %>%
  rename(public_service_commission = NAME)

sc_assignment <- assign_district_by_area(blocks, supreme_court, "NAME") %>%
  rename(supreme_court = NAME)

# Join all assignments to blocks
blocks_clean_area <- blocks %>%
...
# Finds blocks where centroid and area methods disagree
discrepancies <- blocks_clean_area %>%
  left_join(
...
write_csv(blocks_clean_centroid, "cleaned_data/blocks_centroid")
write_csv(blocks_clean_area, "cleaned_data/blocks_area")